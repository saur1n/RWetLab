...
ggplot(df) +
  geom_point(aes(x = C, y = KIN, col = 'Kinetic'), shape = 19, alpha = 0.7) +
  geom_point(aes(x = C, y = EP, col = 'EndPoint'), shape = 15, alpha = 0.7) +
  labs(title = "Effect of Plate Reader Setting",
       x = "Cuvette OD",
       y = "Path Length Corrected Plate OD") +
  scale_color_manual(name = 'Readout Setting',
                     values = c("Kinetic" = "red", "EndPoint" = "blue")) +
  theme_linedraw()


ggplot(df) +
  geom_abline(linetype = 2, col = 'red', lwd = 1.2) +
  geom_point(aes(x = KIN, y = EP), col = 'darkgreen', size = 3) +
